    ...
    <div className="flex min-h-0 min-w-0 flex-1">
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-background">
        <div className="border-b border-border px-4 py-2">
          <h1 className="truncate text-sm font-semibold">{active?.title ?? "Ask"}</h1>
          <p className="text-xs text-muted-foreground">
            Powered by{" "}
            <span className="font-medium text-foreground">{ollamaSettings.llmModel}</span>
            {" · "}on-device, no cloud
          </p>
        </div>
            ...
            {active && active.messages.length === 0 && (
              <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Choose a suggestion or type your own question. Shift+Enter for a new line.
                </p>
        ...
        <div className="border-t border-border bg-background p-3">
          {!ollamaReachable && (
            <div className="mx-auto mb-2 max-w-[720px] rounded-lg border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-900/90 dark:text-amber-200/90">
              ...
              <code className="rounded bg-amber-500/20 px-1 font-mono">ollama serve</code> or check
              Settings → Models.
            </div>
          )}
